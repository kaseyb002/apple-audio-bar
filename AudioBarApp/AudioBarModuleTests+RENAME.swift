...
extension AudioBarModuleTests {
    ...
    func testPlayerDidUpdateDuration1() {
        ...
        let commands = try! Module.update(for: .playerDidLoadMedia(withDuration: 1), model: &model)
    ...
    func testPlayerDidUpdateDuration2() {
        var model = Model.waitingForPlayerToLoadMedia
        let commands = try! Module.update(for: .playerDidLoadMedia(withDuration: 2), model: &model)
    ...
    func testplayerDidUpdateDurationUnexpectedly1() {
        ...
        XCTAssertThrowsError(try Module.update(for: .playerDidLoadMedia(withDuration: 1), model: &model))
    }
